...
- **MVP-2A/2B/2C/2D/2E are implemented locally as checkpoint commits (2026-09-16), not yet Owner Approved** (D-040/D-042/D-043 have full detail). Together they establish the `POST /v1/internal/crud` ingress/dispatcher pattern (`apps-script/core/crud-domain.ts`, `crud-ingress.ts`) with an explicit `CrudOperation` allowlist, header-addressed repositories (`PlansRepository`, `UsersRepository`, `ApplicationsRepository`) that never expose raw Sheet rows, and full CRUD for Plans (Admin-only writes), Users (Admin role/status/profile writes; any actor may edit their own profile fields; last-active-Admin and self-demotion protections), and Applications (Admin/Agent create, Admin any/Agent-own-Pending/Processor-assigned edits, Admin-only Processor assignment). Plans/Users use an `updated_at`-token concurrency pattern (D-041, since neither has a `version` column); Applications use the frozen `assertCurrentVersion`/`version` column and the frozen `validateTransition` status-transition graph, both called unchanged. Every mutation appends one `Activity_Logs` row; every Application status change additionally appends one `Status_History` row. The BFF mirrors each entity with a `server/crud/routes/*.ts` file and matching `api/*` Vercel handlers, all sharing one `BffResponse<T>` envelope, session-cookie + CSRF double-submit + Origin check on mutations, and the unchanged `BffErrorCode` taxonomy. No frontend/dashboard file was touched by any of these batches. Current totals: 139/139 Apps Script tests, 390/390 total unit tests, `format`/`lint`/`typecheck` clean, `gas:build`/`gas:check` passing, production build scanned clean of secrets, 19/19 Playwright regressions unchanged. Committed locally (not pushed) as `feat(mvp-2a)`, `feat(mvp-2b)`, `feat(mvp-2c)` (which also completes MVP-2D/2E — see D-043).
